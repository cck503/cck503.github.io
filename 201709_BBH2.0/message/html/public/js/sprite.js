...
st.defineShapes = {};
st.defineSprites = {};
st.init = function() { st.playList = data;
    st.initRoot();
    st.initDefs();
    st.initShapes();
...
st.initRoot = function() { st.root = st.createTag("svg");
...
st.filterElement = function(a, b) {
    var d = { "0": "Shadow", "1": "Blur", "2": "Glow", "3": "Bevel" },
        c = function(a) {
    ...
    this.Com = function(a, b, c, d) {
        ...
        return this };
    this.Shadow =
        function(a) { this.Rgb(c(this.base)).Fg(a.blurx, a.blury).Fo(a.dx, a.dy).Rgba(a.strength, a.color, "shadow").Com("over", c(this.base), "shadow", ++this.base) };
    ...
    this.Bevel = function(a) { this.Rgb(c(this.base)).Fg(a.blurx, a.blury).Fo("-" + a.dx, "-" + a.dy).Rgba(a.strength, a.highlightColor, ++this.base).Rgb(c(this.base - 1)).Fg(a.blurx, a.blury).Fo(a.dx, a.dy).Rgba(a.strength, a.color, ++this.base).Com("xor", this.base, c(this.base - 1), "shadow").Com("over", "shadow", c(this.base - 2), ++this.base) };
    this.init = function() {
    ...
    this.attachFilter = function() { this.element.childNodes.length > 0 && this.target.setAttribute("filter", "url(#" + this.id + ")") };
    this.init()
};
st.sprite = function(a, b) { this.placeObjects = a.ShowFrame;
    this.sprites = {};
    this.depths = {};
...
st.sprite.getTagByNode = function(a) { a = a.getAttribute("id").split(":");
...
st.sprite.prototype.replaceShape = function(a) {
    ...
    if (this.depths[a.depth]) {
        ...
        if (c == d.id) return g;
        b.replaceChild(d, g);
        this.sprites[a.depth + ":" + this.depths[a.depth]] && (this.sprites[a.depth + ":" + this.depths[a.depth]] = null, delete this.sprites[a.depth + ":" + this.depths[a.depth]]) } else this.lastDepth ? (c = this.par.getAttribute("id") + ":" + this.lastDepth + ":" + this.depths[this.lastDepth],
        c = document.getElementById(c).nextSibling) : c = b.firstChild, c ? b.insertBefore(d, c) : b.appendChild(d);
    this.depths[a.depth] = a.id;
    return d
};
st.sprite.prototype.addFilters = function(a, b) { a.filter ? a.filter.reset(b) : a.filter = new st.filterElement(a, b) };
st.sprite.prototype.action = function(a) {
    for (var b = 0, d = a.length; b < d; b++) switch (a[b].code) {
        ...
        case 6:
            this.play() } };
st.sprite.prototype.stop = function() { this.pause = !0 };
st.sprite.prototype.gotoFrame = function(a) { this.frame = a;
    this.pause = !0 };
st.sprite.prototype.play = function() { this.pause = !1 };
...
st.sprite.prototype.createShape = function(a) {
    var b = this.getNode(a),
        d = this.par.getAttribute("id") + ":" + a.depth + ":" + a.id; "clipDepth" in a ? (this.clipDepthId = d, this.clipDepth = a.clipDepth, this.clipDepthDepth = a.depth, b = this.createClip(a)) : this.clipDepthId && a.depth > this.clipDepthDepth && a.depth <= this.clipDepth && b.setAttribute("clip-path", "url(#" + this.clipDepthId + ")");
    b.setAttribute("id", d);
    return b };
...
st.sprite.prototype.removeShape = function(a) {
    ...
    return null };
st.sprite.prototype.getShape = function(a) { a = this.par.getAttribute("id") + ":" + a.depth + ":" + a.id;
...
st.sprite.prototype.moveSprite = function(a, b) { a.firstChild.nextSibling.setAttribute("transform", "matrix(" + b + ")") };
st.sprite.prototype.changeOpacity = function(a, b) { a.setAttribute("opacity", b) };
...
st.start = function() { st.setBackground(st.root);
    var a = st.createTag("g");
    a.setAttribute("id", "0");
    st.root.appendChild(a);
    ...
    st.timer = setInterval(function() { b.showFrame() }, st.interval) };
st.stop = function() { clearInterval(st.timer) };
st.resume = function() { st.timer = setInterval(function() { mainMovie.showFrame() }, st.interval) };
st.setBackground = function(a) {
    var b = st.createTag("rect"),
        d = { x: data.FrameRect.xmin, y: data.FrameRect.ymin, width: data.FrameWidth, height: data.FrameHeight, fill: data.BackgroundColor },
        c;
