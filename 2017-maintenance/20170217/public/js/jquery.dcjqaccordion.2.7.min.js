		...
		this.each(function(options) {
			...
			if (defaults.eventType == 'hover') {
				...
				$('li a', obj).click(function(e) {
					...
					$('.dcjq-current-parent').find('ul').show()
				})
			}
			...
			function autoCloseAccordion($parentsLi, $parentsUl) {
				console.log($parentsUl);
				console.log($parentsLi);
				//$('ul', obj).not($parentsUl).attr('test', '213')
				//$('ul', obj).not($parentsUl).not($('#category > ul')).attr('test', '213');
				
				$('ul', obj).not($parentsUl).slideUp(defaults.speed);
				$('a', obj).removeClass(defaults.classActive);
				$('> a', $parentsLi).addClass(defaults.classActive);
				//$('ul', obj).not($parentsUl).not($('#category > ul')).slideUp(defaults.speed);
				//$('a', obj).not($('#category_link')).removeClass(defaults.classActive);
				//$('> a', $parentsLi).not($('#category > ul > a')).addClass(defaults.classActive)
				
				console.log($('li:eq(2)', obj));
				var $cookieLi = $('#category', obj);
				$('> a', $cookieLi).addClass(defaults.classActive);
				var $parentsLi = $cookieLi.parents('li');
				console.log($('> a', $parentsLi));
				$('> a', $parentsLi).addClass(defaults.classActive)
