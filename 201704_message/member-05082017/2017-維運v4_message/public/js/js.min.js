/* js cache file: 2017-03-09 16:53:21 */ 
...
if(emulated!==nonEmulated){window.alert('WARNING: You appear to be using IE'+nonEmulated+' in IE'+emulated+' emulation mode.\nIE emulation modes can behave significantly differently from ACTUAL older versions of IE.\nPLEASE DON\'T FILE BOOTSTRAP BUGS based on testing in IE emulation modes!')}})();var domain=document.domain,restUrl='http://rest.notice.babyhome.com.tw',noticeUrl='http://notice.babyhome.com.tw';if(domain.match(/^dev-/gi)){restUrl='http://dev-rest.notice.babyhome.com.tw';noticeUrl='http://dev-notice.babyhome.com.tw';}
$(window).load(function(){$.when($.getScript(noticeUrl+"/socket.io.min.js"),$.getScript(noticeUrl+"/notice.min.js")).done(function(){});});document.write('<div id="fb-root"></div>');var app_id='591744357571224';window.fbAsyncInit=function(){FB.init({appId:app_id,xfbml:true,version:'v2.3'});};(function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(d.getElementById(id)){return;}
js=d.createElement(s);js.id=id;js.src="//connect.facebook.net/zh_TW/sdk.js";fjs.parentNode.insertBefore(js,fjs);}(document,'script','facebook-jssdk'));$(window).load(function(){if(getParameterByName('code')){fbShare();}
function getParameterByName(name){name=name.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var regex=new RegExp("[\\?&]"+name+"=([^&#]*)"),results=regex.exec(location.search);return results===null?"":decodeURIComponent(results[1].replace(/\+/g," "));}
$('.fb_share').click(function(e){e.preventDefault();fbShare();return false;});function fbShare(){FB.init({appId:app_id,xfbml:true,version:'v2.3'});FB.getLoginStatus(function(response){if(response.status==='connected'){var uid=response.authResponse.userID;var accessToken=response.authResponse.accessToken;login_success(uid);}else{login();}});}
function login(){location.href='https://www.facebook.com/dialog/oauth?client_id='+app_id+'&redirect_uri='+location.href+'&fb_oauth=1';}
function login_success(uid){FB.ui({method:'feed',name:$("#subject_title").val(),link:location.href,picture:'http://event.babyhome.com.tw/FB-dimension/600x315.jpg',caption:'',description:$("#subject_content").val()},function(response){if(response&&response.post_id){$.ajax({type:"POST",url:'/follow/ajax_fb_qpoint',data:'post_id='+response.post_id,dataType:'json',async:true,beforeSend:function(xhr){},success:function(data){if(data.success==false){alert(data.msg);return false;}
else if(data.success==true){}}});}else{alert('åˆ†äº«å°šæœªå®Œæˆå–”!');}});}});$(function(){var pr_follow=$("#pr_follow");var pr_list_status=$("#pr_list_status");if(pr_follow){pr_follow.on('click',function(){var allow_status=(pr_follow.prop('checked'))?'Y':'N';$.ajax({type:"POST",url:'http://'+location.hostname+'/Index/ajax_pr_follow',data:{allow:allow_status},dataType:'json',async:true,beforeSend:function(xhr){},success:function(data){if(data.success){}else{alert(data.msg);}}});});}
if(pr_list_status){pr_list_status.on('click',function(){var allow_status=(pr_list_status.prop('checked'))?'Y':'N';$.ajax({type:"POST",url:'http://'+location.hostname+'/Index/ajax_pr_list_status',data:{allow:allow_status},dataType:'json',async:true,beforeSend:function(xhr){},success:function(data){if(data.success){}else{alert(data.msg);}}});});}});$(function(){if($("#login_url")){$("#login_url").attr("href",'http://member.babyhome.com.tw/login?_url='+encodeURIComponent(location.href));}});jQuery(document).ready(function($){var scroll_top_duration=700,$back_to_top=$('.tab-back2top');$back_to_top.on('click',function(event){event.preventDefault();$('body,html').animate({scrollTop:0,},scroll_top_duration);});$('#user-Affix').affix({offset:{top:136,bottom:function(){return(this.bottom=$('.footer').outerHeight(true))}}})
$('#nav-Affix').affix({offset:{top:136,bottom:function(){return(this.bottom=$('.footer').outerHeight(true))}}})
$('#rapidTop-Affix').affix({offset:{top:500,bottom:function(){return(this.bottom=$('.footer').outerHeight(true))}}})
$('.drop-style .dropdown').hover(function(){$(this).find('.pre-act').addClass("active");$(this).find('.dropdown-menu').stop(true,true).delay(100).fadeIn(300);},function(){$(this).find('.dropdown-menu').stop(true,true).delay(100).fadeOut(300);$(this).find('.pre-act').removeClass("active");});$(".other-opinion").click(function(){$(".txt-span.fade").addClass("in");});$(".opinion").click(function(){$(".txt-span.fade.in").removeClass("in");});$(".to_category").click(function(){var to_category=$(this).attr('category');location.href="/list/"+to_category;});$(".bbcall").click(function(){$("#bbcall").remove();});$(function(){$('[data-toggle="tooltip"]').tooltip()})});if("ontouchstart"in document.documentElement){document.documentElement.className+=" touch";}
jQuery('.touch .closeup-span').each(function(){var div=jQuery(this);div.hover(function(){div.removeClass('no-hover');});jQuery('*').not(div).bind('click',function(){div.addClass('no-hover');});});(function(window,factory){if(typeof define==='function'&&define.amd){return define(['jquery'],function(jQuery){return window.Tour=factory(jQuery);});}else if(typeof exports==='object'){return module.exports=factory(require('jquery'));}else{return window.Tour=factory(window.jQuery);}})(window,function($){var Tour,document;document=window.document;Tour=(function(){function Tour(options){var storage;try{storage=window.localStorage;}catch(_error){storage=false;}
this._options=$.extend({name:'tour',steps:[],container:'body',autoscroll:true,keyboard:true,storage:storage,debug:false,backdrop:false,backdropContainer:'body',backdropPadding:0,redirect:true,orphan:false,duration:false,delay:false,basePath:'',template:'<div class="popover" role="tooltip"> <div class="arrow"></div> <h3 class="popover-title"></h3> <div class="popover-content"></div> <div class="popover-navigation"> <div class="btn-group"> <button class="btn btn-sm btn-default" data-role="prev">&laquo; ä¸Šä¸€å€‹</button> <button class="btn btn-sm btn-default" data-role="next">ä¸‹ä¸€å€‹ &raquo;</button> <button class="btn btn-sm btn-default" data-role="pause-resume" data-pause-text="Pause" data-resume-text="Resume">æš«åœ</button> </div> <button class="btn btn-sm btn-default" data-role="end">ç•¥éŽæ•™å­¸</button> </div> </div>',afterSetState:function(key,value){},afterGetState:function(key,value){},afterRemoveState:function(key){},onStart:function(tour){},onEnd:function(tour){},onShow:function(tour){},onShown:function(tour){},onHide:function(tour){},onHidden:function(tour){},onNext:function(tour){},onPrev:function(tour){},onPause:function(tour,duration){},onResume:function(tour,duration){},onRedirectError:function(tour){}},options);this._force=false;this._inited=false;this._current=null;this.backdrops=[];this;}
...
if(step.backdrop){_this._showOverlayElement(step);}
...
break;case 37:e.preventDefault();if(_this._current>0){return _this.prev();}}};})(this));};Tour.prototype._makePromise=function(result){if(result&&$.isFunction(result.then)){return result;}else{return null;}};Tour.prototype._callOnPromiseDone=function(promise,cb,arg){if(promise){return promise.then((function(_this){return function(e){return cb.call(_this,arg);};})(this));}else{return cb.call(this,arg);}};Tour.prototype._showBackground=function(step,data){var $backdrop,height,pos,width,_base,_i,_len,_ref,_results;height=$(document).height();width=$(document).width();_ref=['top','bottom','left','right'];_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){pos=_ref[_i];$backdrop=(_base=this.backdrops)[pos]!=null?_base[pos]:_base[pos]=$('<div>',{"class":"tour-backdrop "+pos});$(step.backdropContainer).append($backdrop);switch(pos){case'top':_results.push($backdrop.height(data.offset.top>0?data.offset.top:0).width(width).offset({top:0,left:0}));break;case'bottom':_results.push($backdrop.offset({top:data.offset.top+data.height,left:0}).height(height-(data.offset.top+data.height)).width(width));break;case'left':_results.push($backdrop.offset({top:data.offset.top,left:0}).height(data.height).width(data.offset.left>0?data.offset.left:0));break;case'right':_results.push($backdrop.offset({top:data.offset.top,left:data.offset.left+data.width}).height(data.height).width(width-(data.offset.left+data.width)));break;default:_results.push(void 0);}}
return _results;};Tour.prototype._showOverlayElement=function(step){var $backdropElement,elementData;$backdropElement=$(step.backdropElement);if($backdropElement.length===0){elementData={width:0,height:0,offset:{top:0,left:0}};}else{elementData={width:$backdropElement.innerWidth(),height:$backdropElement.innerHeight(),offset:$backdropElement.offset()};$backdropElement.addClass('tour-step-backdrop');if(step.backdropPadding){elementData=this._applyBackdropPadding(step.backdropPadding,elementData);}}
return this._showBackground(step,elementData);};Tour.prototype._hideOverlayElement=function(step){var $backdrop,pos,_ref;$(step.backdropElement).removeClass('tour-step-backdrop');_ref=this.backdrops;for(pos in _ref){$backdrop=_ref[pos];$backdrop.remove();}
...
return true;}else{return obj1===obj2;}};return Tour;})();return Tour;});var common={'domain':document.domain,'host':document.domain.split('.'),'pathname':location.pathname.split('/')};$(function(){$('a[name="goback"]').click(function(){window.history.back();return false;});});function GET(k)
